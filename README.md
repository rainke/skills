...
Supports **OpenCode**, **Claude Code**, **Codex**, and **GitHub Copilot** by default, with more agents configurable via `~/.config/skills/config.json`.
...
| `-a, --agent <agents...>` | <!-- agent-names:start -->Target specific agents (e.g., `claude-code`, `codex`, or a custom agent from `~/.config/skills/config.json`). See [Supported Agents](#supported-agents)<!-- agent-names:end --> |
...
Additional agents can be defined in `~/.config/skills/config.json`:
...
- `name` is the CLI value used with `--agent`
- `displayName` is shown in prompts and lists
- Provide at least one of `projectSkillsDir` or `globalSkillsDir`
- `~` is expanded in `globalSkillsDir`
- Agents using `projectSkillsDir: ".agents/skills"` join the shared universal agent flow
...
The CLI automatically detects built-in agents that are installed locally. Configured agents from `~/.config/skills/config.json`
are treated as available once declared.
...
Configured agents can also use any custom `projectSkillsDir` declared in `~/.config/skills/config.json`.
- `.neovate/skills/`
- `.pochi/skills/`
- `.adal/skills/`
<!-- skill-discovery:end -->
